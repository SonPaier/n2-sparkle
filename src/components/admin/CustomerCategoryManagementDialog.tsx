...
interface SortableCategoryItemProps {
  ...
  editName: string;
  onStartEdit: (cat: CustomerCategory) => void;
  onSaveEdit: () => void;
  onCancelEdit: () => void;
  onEditNameChange: (value: string) => void;
  onDelete: (cat: CustomerCategory) => void;
...
interface CustomerCategoryManagementDialogProps {
  open: boolean;
...
export function CustomerCategoryManagementDialog({
  open,
  onOpenChange,
  instanceId,
  customerCounts,
  onCategoriesChanged,
}: CustomerCategoryManagementDialogProps) {
  ...
  const handleStartEdit = (cat: CustomerCategory) => {
    setEditingId(cat.id);
  ...
  const handleDelete = async (cat: CustomerCategory) => {
    const count = customerCounts[cat.id] || 0;
    if (count > 0) {
      toast.error(`Nie można usunąć kategorii z ${count} klientami. Najpierw usuń przypisania.`);
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Zarządzaj kategoriami klientów</DialogTitle>
        </DialogHeader>
              ...
              >
                {categories.map((category) => (
                  <SortableCategoryItem
                    ...
                    customerCount={customerCounts[category.id] || 0}
                  />
                ))}
              </SortableContext>
            </DndContext>
